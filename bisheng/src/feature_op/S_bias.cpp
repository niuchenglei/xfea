...
// 根据单个字段的内容直接生产特征
ReturnCode S_bias::generate_fea(const LogRecordInterface& record, FeaResultSet& fea_result_set) {
    // 避免出现unused-parameter warning
    XFEA_BISHENG_UNUSED_VAR(record);

    // 计算特征签名及将抽取的特征放入fea_result_set，emit_feature位于SingleSlotFeatureOp
    return SingleSlotFeatureOp::emit_feature("bias", fea_result_set);
}
