...
ReturnCode S_direct_p::init(const ExtractorConfig& extractor_config) {
    // 依赖的字段个数必须为1
...
// 根据单个字段的内容直接生产特征
ReturnCode S_direct_p::generate_fea(const LogRecordInterface& record, FeaResultSet& fea_result_set, bool copy_value) {
    float rnd = ((double)rand()) / RAND_MAX;

    if (_output_ratio > 1.0) 
        _output_ratio = _output_ratio - 1.0;

    if (rnd > _output_ratio)
        return SingleSlotFeatureOp::emit_feature(_name, 0, "0", fea_result_set);
